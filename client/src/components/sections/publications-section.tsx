...
const PublicationsSection = () => {
  ...
  const handleFilterChange = (filterGroup: string, selectedOptions: string[]) => {
    setFilters(prev => ({
      ...prev,
  ...
  const filteredPublications = publications?.filter(pub => {
    const qualityMatch = filters.evidenceQuality.length === 0 || 
      filters.evidenceQuality.includes(pub.evidenceQuality);
    
    const typeMatch = filters.evidenceType.length === 0 || 
      filters.evidenceType.includes(pub.evidenceType);
    
    const regionMatch = filters.region.length === 0 || 
      filters.region.includes(pub.region);
    
    const yearMatch = filters.yearRange.length === 0 || 
  ...
  const prepareChartData = () => {
    ...
    const yearCounts: Record<number, { year: number, infection: number, spillover: number }> = {};
    ...
    <section id="recent-reviews" className="bg-gray-50 py-12 md:py-16">
      <div className="container mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <h3 className="font-montserrat font-semibold text-primary text-lg mb-4">Publications by Year</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="infection" name="Evidence of Infection" fill="#002D72" />
                <Bar dataKey="spillover" name="Evidence of Spillover" fill="#418FDE" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <div className="flex items-center">
                ...
                <span className="text-sm text-gray-700">Evidence of Infection</span>
              </div>
              <div className="flex items-center">
                <div className="w-4 h-4 rounded-full bg-blue-400 mr-2"></div>
                <span className="text-sm text-gray-700">Evidence of How Spillover Occurred</span>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex items-center">
                <div className="w-12 h-1 bg-green-600 mr-2"></div>
                <span className="text-sm text-gray-700">High Quality Evidence</span>
              </div>
              <div className="flex items-center">
                <div className="w-12 h-1 bg-yellow-500 mr-2"></div>
                <span className="text-sm text-gray-700">Medium Quality Evidence</span>
              </div>
              <div className="flex items-center">
                <div className="w-12 h-1 bg-red-600 mr-2"></div>
                <span className="text-sm text-gray-700">Low Quality Evidence</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {displayedPublications && displayedPublications.map((publication) => (
            <PublicationCard
              key={publication.id}
              id={publication.id}
        ...
        {filteredPublications && displayedPublications && displayedPublications.length < filteredPublications.length && (
