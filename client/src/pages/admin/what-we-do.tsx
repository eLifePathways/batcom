    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(processFormData)} className="space-y-6">
        ...
        {(contentType === "list" || contentType === "image") && (
          <FormItem>
            <FormLabel>
              {contentType === "list" ? "List Items (JSON)" : "Image Metadata (JSON)"}
            </FormLabel>
            <FormControl>
              <Textarea 
                value={metadataValue || ""}
...
export default function WhatWeDoAdmin() {
  ...
  const [editingSection, setEditingSection] = useState<WhatWeDoSection | undefined>(undefined);
  const [editingContent, setEditingContent] = useState<WhatWeDoContent | undefined>(undefined);
  ...
  const deleteSectionMutation = useMutation({
    mutationFn: (id: number) => apiRequest(`/api/what-we-do/sections/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/what-we-do/sections'] });
  ...
  const createContentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/what-we-do/content/section', selectedSectionId] });
  ...
  const deleteContentMutation = useMutation({
    mutationFn: (id: number) => apiRequest(`/api/what-we-do/content/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/what-we-do/content/section', selectedSectionId] });
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    This will permanently delete the section and all its content.
                                    This action cannot be undone.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel onClick={() => setDeletingSectionId(null)}>
                                    Cancel
                                  </AlertDialogCancel>
                                  <AlertDialogAction 
                                    onClick={() => {
            ...
            {!sections || sections.length === 0 ? (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    You need to create at least one section before adding content.
                  </CardDescription>
                </CardHeader>
                <CardFooter>
              ...
              <>
                ...
                {contentLoading ? (
                  ...
                  <div className="text-center py-10">Select a section to view its content.</div>
                  ...
                  <Card>
                    ...
                    <CardFooter>
                      <Button onClick={openNewContentDialog}>Add Content</Button>
                    </CardFooter>
                  </Card>
                ) : (
                  <div className="rounded-md border">
                                ...
                                {content.contentType === 'image' 
                                  ? (
                                    <div className="flex items-center">
                                      ...
                                      />
                                      <span className="text-xs text-gray-500">{content.content}</span>
                                    </div>
                                  ) 
                                  : content.content
                                }
                              ...
                              <div className="flex space-x-2">
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                      <AlertDialogCancel onClick={() => setDeletingContentId(null)}>
                                        Cancel
                                      </AlertDialogCancel>
                                      <AlertDialogAction 
                                        onClick={() => {
                                          if (deletingContentId) {
                                            handleDeleteContent(deletingContentId);
                                          }
                                        }}
                                      >
                                        Delete
