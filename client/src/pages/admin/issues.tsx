...
export default function IssuesAdmin() {
  ...
  const getStatusBadge = (status: Issue['status']) => {
    switch (status) {
      case 'open':
        return <Badge variant="outline" className="bg-info/10 text-info border-info/30">Open</Badge>;
      case 'in_progress':
      ...
      case 'closed':
        return <Badge variant="outline" className="bg-muted text-foreground border-border">Closed</Badge>;
      default:
  ...
  const getPriorityBadge = (priority: Issue['priority']) => {
    switch (priority) {
      ...
      case 'medium':
        return <Badge variant="outline" className="bg-warning/10 text-warning border-warning/30">Medium</Badge>;
      case 'low':
        return <Badge variant="outline" className="bg-success/10 text-success border-success/30">Low</Badge>;
      default:
        return <Badge variant="outline">Unknown</Badge>;
    }
  };
      ...
      {isLoading ? (
        ...
        <div className="border rounded-lg overflow-hidden">
          <Table>
            ...
            <TableBody>
              {issues.map((issue) => (
                <TableRow key={issue.id} className="cursor-pointer hover:bg-muted" onClick={() => handleViewDetails(issue)}>
                  ...
                  <TableCell className="font-medium truncate max-w-[280px]">
                    <div className="flex items-center">
                      {issueCommentsMap[issue.id]?.hasUnreadComments && (
                        <span className="mr-2 h-2 w-2 rounded-full bg-info animate-pulse" 
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => handleViewDetails(issue)}>
                            ...
                            <div className="flex items-center">
                              View Details
                              {issueCommentsMap[issue.id]?.hasUnreadComments && (
                                <span className="ml-2 h-2 w-2 rounded-full bg-info animate-pulse" />
                              )}
                            </div>
                          </DropdownMenuItem>
                          
                          <DropdownMenuItem onClick={() => handleStatusChange(issue.id, 'open')}>
                            <AlertTriangle className="h-4 w-4 mr-2 text-info" />
                            Mark as Open
                          </DropdownMenuItem>
