...
const ContentRenderer = ({ content }: { content: WhatWeDoContent }) => {
  ...
  switch (content.contentType) {
    case 'text':
      return (
        <div className="mb-8">
          {content.title && <h3 className="text-xl font-semibold text-primary mb-3">{content.title}</h3>}
      ...
      return (
        <div className="mb-8">
          ...
          <div className="relative">
            ...
            />
            {metadata?.caption && (
              <p className="text-sm text-gray-500 mt-2 italic">{metadata.caption}</p>
            )}
          </div>
        </div>
      );
    case 'heading':
      return <h3 className="text-2xl font-semibold text-primary mb-4">{content.content}</h3>;
    case 'list':
      ...
      {isLoading ? (
        ...
        <div>
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className={`grid grid-cols-2 md:grid-cols-${Math.min(sections.length, 5)} w-full`}>
              ...
              {sections.map((section: WhatWeDoSection) => (
                <TabsContent key={section.id} value={section.slug}>
                  <CardHeader>
                    ...
                    {section.subtitle && (
                      <CardDescription className="text-lg">{section.subtitle}</CardDescription>
                    )}
                  </CardHeader>
                  <CardContent>
                    {section.description && (
